...
The first suite replays **committed** evidence — `recordings/bank/m1-acceptance.jsonl`, written by a
real playthrough against `claude-opus-5` — so `pnpm check` re-checks the milestone on a machine with
no credentials, no Python and no browser. It is also the first entry in the regression bank
(ALE-21, `docs/regression-bank.md`): replaying it needs the engine and nothing else, so a later
change to the rules, the hash or the diff set fails immediately against a real model-driven
session. The second regenerates it: it starts this service against the
...
pnpm replay recordings/bank/m1-acceptance.jsonl                          # free
```
