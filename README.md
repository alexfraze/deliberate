...
`off` or `0` disables it) and `GM_SPECULATE_USD` caps what they may cost (default `$0.30`) — a count
alone is not a sum of money, because a preview in combat pays for the NPC turns the game master
takes inside it. Resting the pointer on an action warms the cache for it, so choosing it
