...
from .contracts import ContractError, load_tools, tool_names
...
def create_app(
    ...
    *,
    ...
    state: dict[str, Any] = {"tools": None}

    def resolve_tools() -> list[dict[str, Any]]:
        if state["tools"] is None:
            try:
                contract_tools = load_tools(settings.tools_path)
            except ContractError as exc:
                raise HTTPException(status_code=503, detail=str(exc)) from exc
            # Contract order first, then the service's own tool. Deterministic, so the
            # rendered tool block stays byte-identical and the cache prefix holds.
            state["tools"] = (
                [*contract_tools, PYTHON_TOOL] if settings.python_tool_enabled else contract_tools
            )
    ...
    def healthz() -> dict[str, Any]:
        try:
            tools = tool_names(resolve_tools())
            contract = "loaded"
